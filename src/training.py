...
def compute_metrics(eval_pred):
    """
    Compute entity-level metrics for evaluation using seqeval.

    This function computes proper entity-level F1, precision, and recall scores
    for Named Entity Recognition, treating each complete entity as a unit rather
    than individual BIO tags. This provides more meaningful evaluation metrics
    for NER tasks.
    ...
    Returns:
        Dictionary of entity-level metrics including per-entity-type scores
    """
    ...
    for entity_type, metrics in entity_report.items():
        if entity_type not in ["macro avg", "weighted avg", "micro avg"]:
            result[f"{entity_type}_precision"] = metrics.get("precision", 0.0)
            result[f"{entity_type}_recall"] = metrics.get("recall", 0.0)
            result[f"{entity_type}_f1"] = metrics.get("f1-score", 0.0)
            result[f"{entity_type}_support"] = metrics.get("support", 0.0)
...
class CustomTrainer(Trainer):
    ...
    def _generate_detailed_evaluation_report(self):
        """
        Generate comprehensive evaluation report with entity-level classification metrics.

        Creates detailed classification report and confusion matrix for
        NER evaluation, providing per-entity-type performance metrics and
        visualization of prediction patterns using entity-level evaluation.

        Side Effects:
            - Logs entity-level classification report to experiment tracker
        ...
        try:
            ...
            # Log detailed entity-level metrics
            if self.experiment_tracker:
                self.experiment_tracker.log_classification_report(entity_report)

                # For confusion matrix, we still need flattened labels
                flat_true_labels = [
                    label for sublist in true_reference for label in sublist
                ]
                flat_predictions = [
